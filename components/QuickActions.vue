  ...
  <div class="bg-white/10 backdrop-blur-lg rounded-3xl p-6 border border-white/20">
    <div class="flex items-center justify-between mb-6">
      <h3 class="text-xl font-bold text-white flex items-center gap-2">
        <div class="w-6 h-6 bg-gradient-to-r from-orange-400 to-red-500 rounded-lg flex items-center justify-center">
          ...
          >
            <div class="flex items-center gap-3">
              <div class="w-6 h-6 flex items-center justify-center">
                <component :is="'span'" v-html="format.icon" class="text-gray-300"></component>
              </div>
              <div>
    ...
    <!-- Status Messages -->
    <div v-if="statusMessage" class="mt-4 p-3 rounded-xl" :class="statusMessage.type === 'success' ? 'bg-green-500/20 border border-green-400/30' : 'bg-red-500/20 border border-red-400/30'">
      <p class="text-sm" :class="statusMessage.type === 'success' ? 'text-green-300' : 'text-red-300'">
        {{ statusMessage.text }}
      </p>
    </div>
  </div>
...
const saveFavorites = () => {
    ...
    try {
        ...
        localStorage.setItem(`sporky-favorites-${Date.now()}`, JSON.stringify(favorites));
