  ...
  >
    <div class="flex items-center justify-between mb-6">
      <h3 class="text-xl font-bold text-white flex items-center gap-2">
        <div
          class="w-6 h-6 bg-gradient-to-r from-purple-400 to-pink-400 rounded-lg flex items-center justify-center"
        >
          <svg
    ...
    <div v-else class="space-y-4">
      ...
      >
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-center gap-3">
            <span
              class="flex items-center justify-center w-6 h-6 text-xs font-bold text-white bg-gradient-to-r from-purple-500 to-pink-500 rounded-full"
            >
              {{ index + 1 }}
            </span>
          ...
          <span class="text-sm text-gray-400"
            >{{ artist.count }} track{{ artist.count !== 1 ? 's' : '' }}</span
          >
        </div>
      ...
      <!-- Extended List -->
      <div
        v-if="showAll && artistsData.length > 5"
      ...
      >
        ...
        >
          <div class="flex items-center gap-3">
            <span
              class="flex items-center justify-center w-5 h-5 text-xs text-gray-400 bg-gray-700 rounded-full"
            >
              {{ index + 6 }}
            </span>
            <span class="text-gray-300 text-sm truncate max-w-[180px]">{{
              artist.name
            }}</span>
          </div>
...
const artistsData = computed(() => {
  ...
  const artistsArray = Array.from(artistCounts.entries()).map(
    ([name, count]) => ({
      name,
      count,
    }),
